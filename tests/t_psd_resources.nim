...
test "thumbnail absent or skipped gives none":
  ...
  check not ImageResources(blocks: @[]).hasThumbnail()
  let empty = ImageResources(blocks: @[
    ResourceBlock(id: 1036, name: "", data: @[]),
  ])
  check empty.getThumbnail().isNone
  check not empty.hasThumbnail()

test "hasThumbnail and save round-trip":
  let jpeg = @[byte(0xFF), 0xD8, 0x01, 0x02, 0xFF, 0xD9]
  let res = ImageResources(blocks: @[
    ResourceBlock(id: 1036, name: "", data: thumbPayload(8, 6, jpeg)),
  ])
  check res.hasThumbnail()
  let t = res.getThumbnail().get
  let path = getTempDir() / "psd_test_thumb.jpg"
  t.saveThumbnailJpeg(path)
  let back = readFile(path)
  check back.len == jpeg.len
  for i, v in jpeg:
    check byte(back[i]) == v
  # convenience overload writes the same bytes
  let path2 = getTempDir() / "psd_test_thumb2.jpg"
  res.saveThumbnailJpeg(path2)
  check readFile(path2).len == jpeg.len

test "saveThumbnailJpeg raises without thumbnail":
  expect(PsdError):
    ImageResources(blocks: @[]).saveThumbnailJpeg(getTempDir() / "psd_test_nothumb.jpg")
  expect(PsdError):
    Thumbnail(format: 1, width: 8, height: 6, bitsPerPixel: 24,
      planes: 1, jpeg: @[]).saveThumbnailJpeg(getTempDir() / "psd_test_emptythumb.jpg")

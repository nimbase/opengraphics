...
test "real file thumbnail and icc":
  let doc = openPsd(Fixture)
  check doc.resources.hasThumbnail()
...
test "real file thumbnail saves as JPEG":
  let doc = openPsd(Fixture)
  let path = getTempDir() / "psd_01_thumb.jpg"
  doc.resources.saveThumbnailJpeg(path)
  let d = readFile(path)
  check d.len == 4398
  check byte(d[0]) == 0xFF and byte(d[1]) == 0xD8
  check byte(d[^2]) == 0xFF and byte(d[^1]) == 0xD9
...
test "real file composite saves as BMP":
  let doc = openPsd(Fixture)
  let path = getTempDir() / "psd_01_preview.bmp"
  doc.composite.saveBmp(path)
  let d = readFile(path)
  check d.len == 54 + bmpStride(700) * 700
